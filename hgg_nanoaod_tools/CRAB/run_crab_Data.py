...
config.Site.storageSite          = 'T3_US_NotreDame'
    ...
    path = f"{cfg.replace('_cfg','').replace('.py','')}_submit.py"
...
def run_crab(args):
    ...
    for year in ["2018A", "2018B", "2018C", "2018D"]:
        for dataset in datasets["Data"][f"{year}"]["files"]:
            ...
            subprocess.run(["cmsDriver.py", "--python_filename", path, "--eventcontent", "NANOAOD", "--customise", "Configuration/DataProcessing/Utils.addMonitoring", "--datatier", "NANOAOD", "--fileout", f"file:{cfg}_data.root", "--conditions", f"{GT}", "--step", "NANO", "--filein", f"dbs:{dataset}", "--era", f"Run2_{year[:-1]},{era}", "--data", "-n", "-1", "--no_exec", "--nThreads", "1"])
